  ...
  module Part2
    ...
    def self.walk(vis, map, grid, dist, x, y, dir, dir_steps, visited)
      ...
      [[x - 1, y], [x, y - 1], [x + 1, y], [x, y + 1]].each_with_index do |(xx, yy), id|
        if dir_steps < 3
          nexts << { x: xx, y: yy, dir: id, val: map["#{xx}:#{yy}"] } if id == dir
        else
          next if dir == id && dir_steps == 9
    ...
    def self.vis(grid, x, y)
      Visualisation.print_grid(grid, centre_x: x, centre_y: y, x_dim: 20, y_dim: 20, sleep: 0.01, empty_char: '', spacer: ' ', character_colours: { 'v' => :red, '<' => :red, '>' => :red, '^' => :red }, no_clear: false)
    end
  end
end
